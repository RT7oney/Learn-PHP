...
class WxPayApi {
	...
	public static function unifiedOrder($inputObj, $timeOut = 6) {
		...
		if (!$inputObj->IsNotify_urlSet()) {
			$inputObj->SetNotify_url(env('WECHAT_PAY_NOTIFY_URL')); //异步通知url
		}

		$inputObj->SetAppid(env('WECHAT_APP_ID')); //公众账号ID
		$inputObj->SetMch_id(env('WECHAT_MCH_ID')); //商户号
	...
	private static function postXmlCurl($xml, $url, $useCert = false, $second = 30) {
		...
		if ($useCert == true) {
			...
			curl_setopt($ch, CURLOPT_SSLCERT, env('WECHAT_SSLCERT_PATH'));
			curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
			curl_setopt($ch, CURLOPT_SSLKEY, env('WECHAT_SSLKEY_PATH'));
		}
		//post提交方式
