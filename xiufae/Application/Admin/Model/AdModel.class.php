...
class AdModel extends Model{
    ...
    public function update(){
        ...
        if(empty($data['id'])){
            $res = $this->add();
			Storage::put('Public/Ad/'.I('url').'.js',$this->t2js(stripslashes(I('content'))));
			//记录行为
			action_log('add_ad','ad',$res,UID);
        }else{
            $res = $this->save();
			$info=$this->info($data['id']);
			Storage::put('Public/Ad/'.$info['url'].'.js',$this->t2js(stripslashes(I('content'))));
			//记录行为
	...
	protected function js2t($str){
		$str = str_replace(array("document.write('", "');"), array('', ''), stripcslashes($str));
		return $str;
	}
}