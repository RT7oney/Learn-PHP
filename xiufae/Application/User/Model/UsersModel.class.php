...
class UsersModel extends Model {
	...
	public function info($uid){
		$map['id'] = $uid;
		$map['status']=1;
		$info=$this->where($map)->field('id,username,email,path,last_login_time,introduction,integral,gender,sign')->find();
		$info['follow']=D('Follow')->followCount($uid);
		$info['fans']=D('Follow')->fansCount($uid);
		$info['cfollow']=D('Follow')->checkFollow($uid);
		return $info;
	}
