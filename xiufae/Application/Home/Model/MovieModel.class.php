...
class MovieModel extends Model{
	...
	public function playInfo($id,$order='asc'){
		$prefix   = C('DB_PREFIX');
		$info=M("movie_url")->table($prefix.'movie_url url,'.$prefix.'player play')->where('url.movie_player_id=play.id and url.movie_id='.$id.' and play.display>0')->field('url.id,url.movie_url,url.movie_player_id,play.title')->order("play.sort asc")->select();	
		if(!(is_array($info))){
		...
		foreach($info as $key=>$value){
			if($order=='desc'){
				$info[$key]["movie_url"]=array_reverse($this->playReplace($value["movie_url"],$id,$info[$key]["id"]));
			}else{
				$info[$key]["movie_url"]=$this->playReplace($value["movie_url"],$id,$info[$key]["id"]);
			}
		}
		return $info;
	}
	
	public function playReplace($purl,$id,$pid){
	...
	public function getPlayerUrl($id,$pid,$n){
		$map['id']=$pid;
