...
class TagModel extends Model{
	...
	public function getNav($category,$limit,$type,$field){
		...
		$lists=M("Category")->field($field)->where($map)->limit($limit)->order('sort')->cache(true)->select();
		if($lists){
	...
	public function lists($category,$order,$limit,$status=1,$field=true,$pos=false){
		...
		$lists = M("Movie")->field($field)->where($map)->limit($limit)->order($order)->cache(true)->select();
		if($lists){
			foreach ($lists as $k=>$v){
	...
	public function movieChange($date,$type='movie',$hits=0){
		...
		$date["curl"]=url_change("lists/index",array("id"=>$date["category"],"name"=>$date["ctitle"]));
		if($type=="movie"){
