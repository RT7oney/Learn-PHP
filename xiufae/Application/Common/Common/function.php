...
function url_change($model,$params,$createl=false){
	if(C("WEB_PATTEM")==2 && $model!="lists/lists" && $model!="Search/index"){
		if(C("HTML_PLAYER")==1 and $params['name']=="player" and $createl==false){
			$params['id']=D("Movie")->getmid($params['pid']);
		}
		if(C("HTML_PLAYER")==0 and $params['name']=="player"){
			unset($params['name']);
		...
		switch(C("HTML_RULE")){
		case 1:
			$reurl = __ROOT__."/".C('HTML_PATH').str_replace(array('[name]','[id]','[p]'),array($params['name'],$params['id'],$params['p']),"[name]_[id]_[p]");
			if(!$params['p'])
		  		$reurl = substr($reurl, 0, -1);
			if($createl==false)
				$reurl.=C("HTML_FILE_SUFFIX");
		break;
		case 2:
		  $reurl =__ROOT__."/".C('HTML_PATH').str_replace(array('[dir]','[id]','[p]'),array($params['name'],$params['id'],$params['p']),"[dir]/[id]/[p]/");
		  	$reurl =str_replace(array('//'),array('/'),$reurl);
			if($createl){
				$reurl .= 'index';
			}
		break;
		...
		if(C("HTML_PLAYER")==1 and $params['name']=="player" and $createl==false){
			$reurl.="index".C("HTML_FILE_SUFFIX")."?n=".$params['n']."&pid=".$params['pid'];
		}
	}elseif(C("WEB_PATTEM")==3 && $model!="player/player"){
